 ...
 * @discussion tested
 */
nobleBindings.on('kCBMsgId57', function(args) {
 ...
 * @discussion tested
 */
nobleBindings.on('kCBMsgId58', function(args) {
  var deviceUuid = args.kCBMsgArgDeviceUUID.toString('hex');
...
nobleBindings.on('kCBMsgId60', function(args) {
  if(args.kCBMsgArgResult===307){
 ...
 * @discussion tested
 */
nobleBindings.on('kCBMsgId100', function(args) {
  var deviceUuid = args.kCBMsgArgDeviceUUID.toString('hex');
 ...
 * @discussion tested
 */
nobleBindings.on('kCBMsgId101', function(args) {
  var deviceUuid = args.kCBMsgArgDeviceUUID.toString('hex');
  var serviceUuids = [];
 ...
 * @discussion tested
 */
nobleBindings.on('kCBMsgId109', function(args) {
  var deviceUuid = args.kCBMsgArgDeviceUUID.toString('hex');
  var serviceStartHandle = args.kCBMsgArgServiceStartHandle;
  var serviceUuid = this._peripherals[deviceUuid].services[serviceStartHandle].uuid;
  var result = args.kCBMsgArgResult;
 ...
 * @discussion tested
 */
nobleBindings.on('kCBMsgId115', function(args) {
  var deviceUuid = args.kCBMsgArgDeviceUUID.toString('hex');
  ...
  if (peripheral) {
    ...
    console.warn('noble (catalina): received read event from unknown peripheral: ' + deviceUuid + ' !');
  }
});
 ...
 * @discussion tested
 */
nobleBindings.on('kCBMsgId118', function(args) {
  var deviceUuid = args.kCBMsgArgDeviceUUID.toString('hex');
  var characteristicHandle = args.kCBMsgArgCharacteristicHandle;
  var result = args.kCBMsgArgResult;
