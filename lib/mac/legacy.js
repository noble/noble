...
const debug = require('debug')('legacy-bindings');

const osRelease = os.release();
const isLessThan10_8_5 = (parseFloat(osRelease) < 12.5);
...
nobleBindings.startScanning = function(serviceUuids, allowDuplicates) {
  ...
  if (serviceUuids) {
    for(let i = 0; i < serviceUuids.length; i++) {
      args.kCBMsgArgUUIDs[i] = Buffer.from(serviceUuids[i], 'hex');
    }
  }
...
nobleBindings.on(isLessThan10_8_5 ? 'kCBMsgId13' : 'kCBMsgId31', function(args) {
  const peripheralUuid = args.kCBMsgArgPeripheral.kCBMsgArgUUID.toString('hex');
  const peripheralHandle = args.kCBMsgArgPeripheral.kCBMsgArgPeripheralHandle;
  const advertisement = {
  ...
  (function(peripheralUuid, peripheralHandle, advertisement, rssi) {
    uuidToAddress(peripheralUuid, function(error, address = 'unknown', addressType = 'unknown') {
...
nobleBindings.on(isLessThan10_8_5 ? 'kCBMsgId14' : 'kCBMsgId32', function(args) {
  const peripheralUuid = args.kCBMsgArgUUID.toString('hex');
...
nobleBindings.on(isLessThan10_8_5 ? 'kCBMsgId20' : 'kCBMsgId41', function(args) {
  const peripheralHandle = args.kCBMsgArgPeripheralHandle;
  const peripheralUuid = this._peripherals[peripheralHandle].uuid;
...
nobleBindings.discoverServices = function(peripheralUuid, uuids) {
  ...
  if (uuids) {
    for(let i = 0; i < uuids.length; i++) {
      args.kCBMsgArgUUIDs[i] = Buffer.from(uuids[i], 'hex');
    }
  }
...
nobleBindings.on(isLessThan10_8_5 ? 'kCBMsgId21' : 'kCBMsgId42', function(args) {
  ...
  for(let i = 0; i < args.kCBMsgArgServices.length; i++) {
    const service = {
...
nobleBindings.on(isLessThan10_8_5 ? 'kCBMsgId27' : 'kCBMsgId48', function(args) {
  const peripheralUuidHandle = args.kCBMsgArgPeripheralHandle;
  const peripheralUuid = this._peripherals[peripheralUuidHandle].uuid;
  const serviceStartHandle = args.kCBMsgArgServiceStartHandle;
  const serviceUuid = this._peripherals[peripheralUuidHandle].services[serviceStartHandle].uuid;
  ...
  for(let i = 0; i < args.kCBMsgArgServices.length; i++) {
    ...
    this._peripherals[peripheralUuidHandle].services[serviceStartHandle].includedServices[includedService.uuid] =
...
nobleBindings.discoverCharacteristics = function(peripheralUuid, serviceUuid, characteristicUuids) {
  ...
  if (characteristicUuids) {
    for(let i = 0; i < characteristicUuids.length; i++) {
      args.kCBMsgArgUUIDs[i] = Buffer.from(characteristicUuids[i], 'hex');
    }
  }
...
nobleBindings.on(isLessThan10_8_5 ? 'kCBMsgId28' : 'kCBMsgId49', function(args) {
  ...
  const peripheralUuid = this._peripherals[peripheralHandle].uuid;
  const serviceStartHandle = args.kCBMsgArgServiceStartHandle;
  const serviceUuid = this._peripherals[peripheralHandle].services[serviceStartHandle].uuid;
  const characteristics = [];

  this._peripherals[peripheralHandle].services[serviceStartHandle].characteristics = {};

  for(let i = 0; i < args.kCBMsgArgCharacteristics.length; i++) {
    const properties = args.kCBMsgArgCharacteristics[i].kCBMsgArgCharacteristicProperties;
...
nobleBindings.on(isLessThan10_8_5 ? 'kCBMsgId35' : 'kCBMsgId56', function(args) {
  const peripheralHandle = args.kCBMsgArgPeripheralHandle;
  const peripheral = this._peripherals[peripheralHandle];
...
nobleBindings.on(isLessThan10_8_5 ? 'kCBMsgId36' : 'kCBMsgId57', function(args) {
  ...
  for(const i in this._peripherals[peripheralHandle].services) {
...
nobleBindings.on(isLessThan10_8_5 ? 'kCBMsgId39' : 'kCBMsgId60', function(args) {
  ...
  for(const i in this._peripherals[peripheralHandle].services) {
    if (this._peripherals[peripheralHandle].services[i].characteristics &&
      ...
      for(let j = 0; j < args.kCBMsgArgDescriptors.length; j++) {
        const descriptor = {
...
nobleBindings.on(isLessThan10_8_5 ? 'kCBMsgId42' : 'kCBMsgId63', function(args) {
  ...
  for(const i in this._peripherals[peripheralHandle].services) {
    for(const j in this._peripherals[peripheralHandle].services[i].characteristics) {
