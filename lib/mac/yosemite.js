...
nobleBindings.on('kCBMsgId56', function(args) {
  ...
  if (args.kCBMsgArgServices) {
    for(var i = 0; i < args.kCBMsgArgServices.length; i++) {
      ...
      this._peripherals[deviceUuid].services[service.id] = service;
  ...
  this.emit('servicesDiscover', deviceUuid, services);
});
...
nobleBindings.on('kCBMsgId63', function(args) {
  ...
  for(var i = 0; i < args.kCBMsgArgServices.length; i++) {
    ...
    includedServiceUuids.push({
      id: includedService.id,
      uuid: includedService.uuid
    });
  }

  this.emit('includedServicesDiscover', deviceUuid, serviceId, includedServiceUuids);
});
...
nobleBindings.on('kCBMsgId64', function(args) {
  ...
  for(var i = 0; i < args.kCBMsgArgCharacteristics.length; i++) {
    ...
    this._peripherals[deviceUuid].services[serviceId].characteristics[characteristic.handle] =
      this._peripherals[deviceUuid].services[serviceId].characteristics[characteristic.valueHandle] = characteristic;
  ...
  this.emit('characteristicsDiscover', deviceUuid, serviceId, characteristics);
});
  ...
  for(var i in this._peripherals[deviceUuid].services) {
    if (this._peripherals[deviceUuid].services[i].characteristics &&
      ...
      for(var j = 0; j < args.kCBMsgArgDescriptors.length; j++) {
        ...
        this._peripherals[deviceUuid].services[i].characteristics[characteristicHandle].descriptors[descriptor.id] = descriptor;
