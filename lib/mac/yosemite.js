...
nobleBindings.on('kCBMsgId37', function(args) {
  ...
  (function(deviceUuid, advertisement, rssi) {
    uuidToAddress(deviceUuid, function(error, address, addressType) {
      ...
      self._peripherals[deviceUuid].address = address;
      self._peripherals[deviceUuid].addressType = addressType;

      self.emit('discover', deviceUuid, address, addressType, connectable, advertisement, rssi);
    });
