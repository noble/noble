...
Gatttool.prototype.onStdoutData = function(data) {
  ...
  this._buffer += string; //.replace(/\s*(.*)\s*/g, '$1');
  ...
  while (found = this._buffer.match(/([\s\S]*)\[(.{3})\]\[(.{17})\]\[(.*)\]>(.*)/)) {
    ...
    if (this._currentCommand) {
      ...
      if (!this._currentCommand.async && splitCommandOutput[0].trim() === this._currentCommand.command.trim()) {
        done = true;
      } else if (this._currentCommand.async && splitCommandOutput[0].trim() !== this._currentCommand.command.trim()) {
        done = true;
      }
