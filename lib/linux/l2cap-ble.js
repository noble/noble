...
L2capBle.prototype._queueCommand = function(buffer, callback) {
  ...
  if (this._currentCommand === null) {
    ...
    console.log('out ' + this._currentCommand.buffer.toString('hex') + '\n');
    this._l2capBle.stdin.write(this._currentCommand.buffer.toString('hex') + '\n');

    if (!this._currentCommand) {
      this._currentCommand = null; // TODO: check for next command
    }
  }
};
...
L2capBle.prototype.discoverIncludedServices = function(serviceUuid, includedServiceUuids) {
  ...
  var callback = function(data) {
    ...
    if (opcode === ATT_OP_READ_BY_TYPE_RESP) {
      ...
      for (i = 0; i < num; i++) {
        // TODO: verify data unmarshalling, and only push includedServiceUuids
    ...
    if (opcode !== ATT_OP_READ_BY_TYPE_RESP || includedServices[includedServices.length - 1].endHandle === 0xffff) {
      ...
      this._queueCommand(this.readByTypeRequest(includedServices[includedServices.length - 1].endHandle, service.endHandle, GATT_INCLUDE_UUID), callback);
    }
  }.bind(this);
...
L2capBle.prototype.discoverCharacteristics = function(serviceUuid, characteristicUuids) {
  ...
  var callback = function(data) {
    ...
    if (opcode !== ATT_OP_READ_BY_TYPE_RESP || characteristics[characteristics.length - 1].valueHandle === 0xffff) {
      ...
      this.emit('characteristicsDiscover', this._address, service.uuid, characteristicsDiscovered);
    } else {
      this._queueCommand(this.readByTypeRequest(characteristics[characteristics.length - 1].valueHandle, service.endHandle, GATT_CHARAC_UUID), callback);
    }
  }.bind(this);
...
L2capBle.prototype.write = function(serviceUuid, characteristicUuid, data, withoutResponse) {
  ...
  if (withoutResponse) {
    this._queueCommand(this.writeRequest(characteristic.valueHandle, data, true));
