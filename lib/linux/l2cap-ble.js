...
L2capBle.prototype.onStdoutData = function(data) {
  ...
  while ((newLineIndex = this._buffer.indexOf('\n')) !== -1) {
    ...
    if ((found = line.match(/^connect (.*)$/))) {
      ...
      if (lineData[0] === ATT_OP_HANDLE_NOTIFY) {
  ...
  var callback = function(data) {
    ...
    if (opcode !== ATT_OP_READ_BY_TYPE_RESP || includedServices[includedServices.length - 1].endHandle === 0xffff) {
      ...
      for (i = 0; i < includedServices.length; i++) {
        if (includedServiceUuids.length === 0 || includedServiceUuids.indexOf(includedServices[i].uuid) !== -1) {
...
L2capBle.prototype.write = function(serviceUuid, characteristicUuid, data, withoutResponse) {
  var characteristic = this._characteristics[serviceUuid][characteristicUuid];
console.log('write ' + withoutResponse);
  if (withoutResponse) {
