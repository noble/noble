...
Hci.prototype.writeAclDataPkt = function(handle, code, data) {
...
Hci.prototype.writeAclAttDataPkt = function(handle, data) {
  this.writeAclDataPkt(handle, ATT_CID, data);
};

Hci.prototype.writeSmpDataPkt = function(handle, code, data) {
  this.writeAclDataPkt(handle, SMP_CID, data);
};
...
Hci.prototype.onSocketData = function(data) {
  ...
  if (HCI_EVENT_PKT === eventType) {
    ...
    if (ACL_START === flags && data.readUInt16LE(7) === ATT_CID) {
      ...
      if (length === pktData.length) {
        ...
        this.emit('aclAttDataPkt', handle, pktData);
      } else {
      ...
      if (this._handleBuffers[handle].data.length === this._handleBuffers[handle].length) {
        this.emit('aclAttDataPkt', handle, this._handleBuffers[handle].data);
