        ...
        switch(type) {
          ...
          case 0x03: // Complete List of 16-bit Service Class UUIDs
            for (j = 0; j < bytes.length; j += 2) {
              var uuid = bytes.readUInt16LE(j).toString(16);
              if (advertisement.serviceUuids.indexOf(uuid) === -1) {
                advertisement.serviceUuids.push();
              }
            }
            break;
          ...
          case 0x07: // Complete List of 128-bit Service Class UUIDs
            for (j = 0; j < bytes.length; j += 16) {
              var uuid = bytes.slice(j, j + 16).toString('hex').match(/.{1,2}/g).reverse().join('');
              if (advertisement.serviceUuids.indexOf(uuid) === -1) {
                advertisement.serviceUuids.push();
              }
            }
