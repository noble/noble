...
var Gatt = function(address, aclAttStream) {
  this._address = address;
  this._aclAttStream = aclAttStream;
  ...
  this.onAclAttStreamDataBinded = this.onAclAttStreamData.bind(this);
  this.onAclAttStreamEndBinded = this.onAclAttStreamEnd.bind(this);

  this._aclAttStream.on('data', this.onAclAttStreamDataBinded);
  this._aclAttStream.on('end', this.onAclAttStreamEndBinded);
};

util.inherits(Gatt, events.EventEmitter);


Gatt.prototype.onAclAttStreamData = function(data) {
  if (this._currentCommand && data.toString('hex') === this._currentCommand.buffer.toString('hex')) {
    ...
    if (data[0] === ATT_OP_ERROR &&
      ...
      // TODO: upgrade security, and resend last command
      return;
    }
    ...
    while(this._commandQueue.length) {
      ...
      this._aclAttStream.write(this._currentCommand.buffer);
...
Gatt.prototype.onAclAttStreamEnd = function() {
  this._aclAttStream.removeListener('data', this.onAclAttStreamDataBinded);
  this._aclAttStream.removeListener('end', this.onAclAttStreamEndBinded);
};
