...
BlueZ5.prototype.setupManagedObjects = function() {
  ...
  for(var object in managedObjects) {
    ...
    for(var interface_ in interfaces) {
      var properties = interfaces[interface_];

      if (interface_ === 'org.bluez.Adapter1') {
        this._adapter = this._dbus.get_interface(this._systemBus, 'org.bluez', object, interface_);
        ...
        debug('Adapter = ' + object + ' ' + JSON.stringify(properties));

        this._powered = properties.Powered;
      } else if (interface_ === 'org.bluez.Device1') {
...
BlueZ5.prototype.onInterfacesAdded = function(object, interfaces) {
  for(var interface_ in interfaces) {
    ...
    if (interface_ === 'org.bluez.Device1') {
      debug('Device InterfacesAdded ' + object + ' ' + JSON.stringify(properties));
...
BlueZ5.prototype.onAdapterPropertiesChanged = function(interface_, changedProperties, invalidatedProperties) {
