...
NobleBindings.prototype.connect = function(deviceUuid) {
  ...
  peripheral.device.gatt.connect()
    .then(function(gattServer){
      ...
      self.emit('connect', null, deviceUuid);
    }, function(err){
      debug('err connecting', err);
      self.emit('connect', err, deviceUuid);
    });
...
NobleBindings.prototype.disconnect = function(deviceUuid) {
  ...
  if(peripheral.device.gatt){
    ...
    this.emit('disconnect', null, deviceUuid);
  }
};
...
NobleBindings.prototype.discoverServices = function(deviceUuid, uuids) {
  ...
  //TODO: need web api completed for this to work
  if(peripheral){
    this.emit('servicesDiscover', null, deviceUuid, peripheral.serviceUuids);
  }
  ...
  if(peripheral){
    ...
    self.getPrimaryService(peripheral, serviceUuid)
      ...
      .then(function(characteristics) {
        ...
        self.emit('characteristicsDiscover', null, deviceUuid, serviceUuid, discoveredCharateristcs);
      })
      .catch(function(error) {
        self.emit('characteristicsDiscover', error, deviceUuid, serviceUuid);
      });
  }
...
NobleBindings.prototype.read = function(deviceUuid, serviceUuid, characteristicUuid) {
  ...
  self.getPrimaryService(peripheral, serviceUuid)
    ...
    .then(function(data){
      self.emit('read', null, peripheral.uuid, serviceUuid, characteristicUuid, new Buffer(data.buffer), false);
    })
    .catch(function(error) {
      debug('error reading characteristic', error);
      self.emit('read', error);
    });
};
...
NobleBindings.prototype.write = function(deviceUuid, serviceUuid, characteristicUuid, data, withoutResponse) {
  ...
  self.getPrimaryService(peripheral, serviceUuid)
    ...
    .then(function(){
      debug('value written');
      self.emit('write', null, peripheral.uuid, serviceUuid, characteristicUuid);
    })
    .catch(function(err) {
      self.emit('write', err, peripheral.uuid, serviceUuid, characteristicUuid);
    });
  ...
  if(notify){
    ...
    .then(function(characteristic){
      ...
      peripheral.notifcationListeners[serviceUuid + '__' + characteristicUuid] = function(evt){
        ...
        self.emit('read', null, deviceUuid, serviceUuid, characteristicUuid, new Buffer(evt.target.value.buffer), true);
      };
      ...
      self.emit('notify', null, deviceUuid, serviceUuid, characteristicUuid, true);
      return characteristic;
    })
    .catch(function(err) {
      debug('error enabling notifications on characteristic', err);
      self.emit('notify', err, deviceUuid, serviceUuid, characteristicUuid, true);
    });
  }
  else{
    charPromise.then(function(characteristic) {
    ...
    .then(function(characteristic){
      ...
      self.emit('notify', null, deviceUuid, serviceUuid, characteristicUuid, false);
      return characteristic;
    })
    .catch(function(err) {
      debug('error disabling notifications on characteristic', err);
      self.emit('notify', err, deviceUuid, serviceUuid, characteristicUuid, false);
    });
  }
