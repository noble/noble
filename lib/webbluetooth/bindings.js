...
NobleBindings.prototype.read = function(deviceUuid, serviceUuid, characteristicUuid) {
  ...
  self._getPrimaryService(peripheral, serviceUuid)
    ...
    .then(function(data){
      self.emit('read', peripheral.uuid, serviceUuid, characteristicUuid, new Buffer(characteristic.value.buffer), false);
    })
    .catch(function(err) {
      console.log('error reading characteristc', err);
    });
};
  ...
  self._getPrimaryService(peripheral, serviceUuid)
    ...
    .then(function(characteristic) {
      ...
      characteristic.oncharacteristicvaluechanged = function(evt){
        console.log('oncharacteristicvaluechanged', evt, new Buffer(characteristic.value.buffer));
