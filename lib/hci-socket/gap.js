...
Gap.prototype.onHciLeAdvertisingReport = function(status, type, address, addressType, eir, rssi) {
  ...
  var previousConnectable = (previouslyDiscovered) ? this._discoveries[address].connectable : undefined;
  ...
  // only report after a scan response event or more than one discovery without a scan response, so more data can be collected or if connectability changed
  if ( previousConnectable !== connectable || isNonScannableEventType(type) || type === EVENT_TYPE_SCAN_RESPONSE || (discoveryCount > 1 && !hasScanResponse) || process.env.NOBLE_REPORT_ALL_HCI_EVENTS) {
