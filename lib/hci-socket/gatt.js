...
const UUID_FORMAT_16BITS = 1;
const UUID_FORMAT_128BITS = 2;
const HANDLE_SIZE_IN_BYTES = 2;
const UUID16_ID_SIZE_IN_BYTES = 2;
const UUID128_ID_SIZE_IN_BYTES = 16;
...
Gatt.prototype.extractUUIDs = function(data, uuidSizeInBytes = 2) {
  let descriptors = [];
  let totalSizeInBytesPerUUID = uuidSizeInBytes + HANDLE_SIZE_IN_BYTES;
  let totalPositions = data.length / totalSizeInBytesPerUUID;
  for (let pos = 0; pos < totalPositions; pos++) {
    let positionInBytes = pos * totalSizeInBytesPerUUID;
    let handle = data.readUIntLE(positionInBytes, HANDLE_SIZE_IN_BYTES);
    let uuid = data
      .slice(
        positionInBytes + HANDLE_SIZE_IN_BYTES,
        positionInBytes + totalSizeInBytesPerUUID
      )
      .reverse()
      .toString("hex");
    descriptors.push({
...
Gatt.prototype.discoverDescriptors = function(serviceUuid, characteristicUuid) {
  ...
  var callback = function(data) {
    ...
    if (opcode === ATT_OP_FIND_INFO_RESP) {
      var format = data[1];
      switch (format) {
        case UUID_FORMAT_16BITS:
          descriptors = this.extractUUIDs(
            data.slice(2),
            UUID16_ID_SIZE_IN_BYTES
          );
          break;
        case UUID_FORMAT_128BITS:
          descriptors = this.extractUUIDs(
            data.slice(2),
            UUID128_ID_SIZE_IN_BYTES
          );
          break;
      }
    }
