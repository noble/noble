  ...
  var callback = function(data) {
    ...
    if (opcode === ATT_OP_FIND_INFO_RESP) {
      ...
      switch(format){
        ...
        break;
        case 2:
          var num = (data.length - 2) / 18;
          data = data.slice(2);
          for (var i = 0; i < num; i++) {
            var h = data.readUInt16LE(i * 18);
            var uuidBuf = data.slice(2 + i*18, (i+1)*18);
            var u = "";
            for (var j = 0; j < uuidBuf.length; j++) {
              var newChar = uuidBuf.readUInt8(j).toString(16);
              if (newChar.length < 2)
                newChar = "0" + newChar;
              u = newChar + u;
            }
