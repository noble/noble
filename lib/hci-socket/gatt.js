...
Gatt.prototype.longWrite = function(serviceUuid, characteristicUuid, data, withoutResponse) {
  ...
  var prepareWriteRequest = function(offset, data) {
    this._queueCommand(this.prepareWriteRequest(characteristic.valueHandle, offset, data.slice(0,limit)), function(resp) {
      var opcode = resp[0];
      if (opcode == ATT_OP_PREPARE_WRITE_RESP) {
        if (data.length > limit) {
          prepareWriteRequest(offset + limit, data.slice(limit));
        } else {
