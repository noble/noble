...
Gatt.prototype.discoverServices = function(uuids) {
  ...
  var callback = function(data) {
    ...
    if (opcode !== ATT_OP_READ_BY_GROUP_RESP || services[services.length - 1].endHandle === 0xffff) {
      ...
      this.emit('servicesDiscover', this._address, services_);
    } else {
      ...
      for (i = 0; i < includedServices.length; i++) {
        if (uuids.length === 0 || uuids.indexOf(includedServices[i].uuid) !== -1) {
          includedServices_.push({
            id: includedServices[i].id,
            uuid: includedServices[i].uuid
          });
        }
      }

      this.emit('includedServicesDiscover', this._address, service.id, includedServices_);
    } else {
...
Gatt.prototype.discoverDescriptors = function(serviceId, characteristicId) {
  ...
  this._descriptors[characteristicId][characteristicId] = {};
  ...
  var callback = function(data) {
    ...
    if (opcode !== ATT_OP_FIND_INFO_RESP || descriptors[descriptors.length - 1].handle === characteristic.endHandle) {
      ...
      this.emit('descriptorsDiscover', this._address, serviceId, characteristicId, descriptors_);
    } else {
