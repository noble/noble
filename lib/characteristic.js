...
Characteristic.prototype.read = function(callback) {
  if (callback) {
    this.once('read', function(error, data) {
      callback(error, data);
    });
  }
...
Characteristic.prototype.write = function(data, withoutResponse, callback) {
  ...
  if (callback) {
    this.once('write', function(error) {
      callback(error);
    });
  }
...
Characteristic.prototype.broadcast = function(broadcast, callback) {
  if (callback) {
    this.once('broadcast', function(error) {
      callback(error);
    });
  }
...
Characteristic.prototype.notify = function(notify, callback) {
  if (callback) {
    this.once('notify', function(error) {
      callback(error);
    });
  }
...
Characteristic.prototype.discoverDescriptors = function(callback) {
  if (callback) {
    this.once('descriptorsDiscover', function(error, descriptors) {
