...
Peripheral.prototype.disconnect = function(callback) {
  if (callback) {
    this.once('disconnect', function(error) {
      callback(error);
    });
  }
...
Peripheral.prototype.updateRssi = function(callback) {
  if (callback) {
    this.once('rssiUpdate', function(error, rssi) {
      callback(error, rssi);
    });
  }
...
Peripheral.prototype.discoverServices = function(uuids, callback) {
  if (callback) {
    this.once('servicesDiscover', function(error, services) {
...
Peripheral.prototype.readHandle = function(handle, callback) {
  if (callback) {
    this.once('handleRead' + handle, function(error, data) {
      callback(error, data);
    });
  }
...
Peripheral.prototype.writeHandle = function(handle, data, withoutResponse, callback) {
  ...
  if (callback) {
    this.once('handleWrite' + handle, function(error) {
      callback(error);
    });
  }
