...
Noble.prototype.startScanning = function(serviceUuids, allowDuplicates, callback) {
  if (this.state !== 'poweredOn') {
    ...
    if (callback) {
      this.once('scanStart', function(error, filterDuplicates) {
        callback(error, filterDuplicates);
      });
    }
...
Noble.prototype.onScanStart = function(error, filterDuplicates) {
  if (error) return this.emit('scanStart', error);
...
Noble.prototype.onScanStop = function(error) {
  if (error) return this.emit('scanStop', error);
...
Noble.prototype.onConnect = function(error, peripheralUuid) {
...
Noble.prototype.onRssiUpdate = function(error, peripheralUuid, rssi) {
  ...
  if (peripheral) {
    ...
    peripheral.emit('rssiUpdate', error, rssi);
  } else {
...
Noble.prototype.onServicesDiscover = function(error, peripheralUuid, serviceUuids) {
  ...
  if (peripheral) {
    if (error) return peripheral.emit('servicesDiscover', error);
...
Noble.prototype.onIncludedServicesDiscover = function(error, peripheralUuid, serviceUuid, includedServiceUuids) {
  ...
  if (service) {
    if (error) return service.emit('includedServicesDiscover', error);
...
Noble.prototype.onCharacteristicsDiscover = function(error, peripheralUuid, serviceUuid, characteristics) {
  ...
  if (service) {
    if (error) return service.emit('characteristicsDiscover', error);
...
Noble.prototype.onRead = function(error, peripheralUuid, serviceUuid, characteristicUuid, data, isNotification) {
  ...
  if (characteristic) {
      characteristic.emit('data', error, data, isNotification);

      if (!isNotification) {
        characteristic.emit('read', error, data, isNotification); // for backwards compatbility
      }
  } else {
...
Noble.prototype.onBroadcast = function(error, peripheralUuid, serviceUuid, characteristicUuid, state) {
  ...
  if (characteristic) {
    characteristic.emit('broadcast', error, state);
  } else {
...
Noble.prototype.onNotify = function(error, peripheralUuid, serviceUuid, characteristicUuid, state) {
  ...
  if (characteristic) {
    characteristic.emit('notify', error, state);
  } else {
...
Noble.prototype.onDescriptorsDiscover = function(error, peripheralUuid, serviceUuid, characteristicUuid, descriptors) {
  ...
  if (characteristic) {
    if (error) return characteristic.emit('descriptorsDiscover', error);
...
Noble.prototype.onValueRead = function(error, peripheralUuid, serviceUuid, characteristicUuid, descriptorUuid, data) {
  ...
  if (descriptor) {
    descriptor.emit('valueRead', error, data);
  } else {
...
Noble.prototype.onHandleRead = function(error, peripheralUuid, handle, data) {
  ...
  if (peripheral) {
    peripheral.emit('handleRead' + handle, error, data);
  } else {
...
Noble.prototype.onHandleNotify = function(error, peripheralUuid, handle, data) {
  ...
  if (peripheral) {
    peripheral.emit('handleNotify', error, handle, data);
  } else {
