...
function Noble(bindings) {
  ...
  this._alwaysClearPeripheralData = false;
...
Noble.prototype.startScanning = function(serviceUuids, allowDuplicates, callback) {
  var scan = function(state) {
    if (state !== 'poweredOn') {
      ...
      this._bindings.startScanning(serviceUuids, allowDuplicates, this._alwaysClearPeripheralData);
    }
  };
...
Noble.prototype.onDiscover = function(uuid, address, addressType, connectable, advertisement, rssi) {
  if (this._alwaysClearPeripheralData) {
    this._peripherals = {}
  }
...
Noble.prototype.onValueRead = function(peripheralUuid, serviceUuid, characteristicUuid, descriptorUuid, data) {
  ...
  // Safely iterate through nested object to find descriptor if exists
  let peripheral = this._descriptors[peripheralUuid]
  if (peripheral != undefined) {

    let service = peripheral[serviceUuid]
    if (service != undefined) {

      let characteristic = service[characteristicUuid]
      if (characteristic != undefined) {

        let descriptor = characteristic[descriptorUuid]
