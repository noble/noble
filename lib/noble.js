...
Noble.prototype.onConnect = function(peripheralUuid, error) {
  ...
  if (peripheral) {
    ...
    peripheral.emit('_connect', error);
  } else {
...
Noble.prototype.onDisconnect = function(peripheralUuid) {
  ...
  if (peripheral) {
    ...
    var disconnectedError = new Error('Peripheral disconnected!');

    // fail any pending operations
    peripheral.emit('_rssiUpdate', disconnectedError);
    peripheral.emit('_servicesDiscover', disconnectedError);

    if (peripheral.services) {
      for (var i = 0; i < peripheral.services.length; i++) {
        var service = peripheral.services[i];

        service.emit('_includedServicesDiscover', disconnectedError);
        service.emit('_characteristicsDiscover', disconnectedError);

        if (service.characteristics) {
          for (var j = 0; j < service.characteristics.length; j++) {
            var characteristic = service.characteristics[j];

            characteristic.emit('_read', disconnectedError);
            characteristic.emit('_write', disconnectedError);
            characteristic.emit('_broadcast', disconnectedError);
            characteristic.emit('_notify', disconnectedError);
            characteristic.emit('_descriptorsDiscover', disconnectedError);

            if (characteristic.descriptors) {
              for (var k = 0; k < characteristic.descriptors.length; k++) {
                var descriptor = characteristic.descriptors[k];

                descriptor.emit('_valueRead', disconnectedError);
                descriptor.emit('_valueWrite', disconnectedError);
              }
            }
          }
        }
    ...
    peripheral.emit('_disconnect', null);
  } else {
...
Noble.prototype.onRssiUpdate = function(peripheralUuid, rssi) {
  ...
  if (peripheral) {
    ...
    peripheral.emit('_rssiUpdate', null, rssi);
  } else {
...
Noble.prototype.onServicesDiscover = function(peripheralUuid, serviceUuids) {
  ...
  if (peripheral) {
    ...
    peripheral.emit('_servicesDiscover', null, services);
  } else {
...
Noble.prototype.onIncludedServicesDiscover = function(peripheralUuid, serviceUuid, includedServiceUuids) {
  ...
  if (service) {
    ...
    service.emit('_includedServicesDiscover', null, includedServiceUuids);
  } else {
...
Noble.prototype.onCharacteristicsDiscover = function(peripheralUuid, serviceUuid, characteristics) {
  ...
  if (service) {
    ...
    service.emit('_characteristicsDiscover', null, characteristics_);
  } else {
...
Noble.prototype.onRead = function(peripheralUuid, serviceUuid, characteristicUuid, data, isNotification) {
  ...
  if (characteristic) {
    characteristic.emit('data', data, isNotification); // no internal event

    characteristic.emit('_read', null, data, isNotification); // for backwards compatbility
  } else {
...
Noble.prototype.onWrite = function(peripheralUuid, serviceUuid, characteristicUuid) {
  ...
  if (characteristic) {
    characteristic.emit('_write', null);
  } else {
...
Noble.prototype.onBroadcast = function(peripheralUuid, serviceUuid, characteristicUuid, state) {
  ...
  if (characteristic) {
    characteristic.emit('_broadcast', null, state);
  } else {
...
Noble.prototype.onNotify = function(peripheralUuid, serviceUuid, characteristicUuid, state) {
  ...
  if (characteristic) {
    characteristic.emit('_notify', null, state);
  } else {
...
Noble.prototype.onDescriptorsDiscover = function(peripheralUuid, serviceUuid, characteristicUuid, descriptors) {
  ...
  if (characteristic) {
    ...
    characteristic.emit('_descriptorsDiscover', null, descriptors_);
  } else {
...
Noble.prototype.onValueRead = function(peripheralUuid, serviceUuid, characteristicUuid, descriptorUuid, data) {
  ...
  if (descriptor) {
    descriptor.emit('_valueRead', null, data);
  } else {
...
Noble.prototype.onValueWrite = function(peripheralUuid, serviceUuid, characteristicUuid, descriptorUuid) {
  ...
  if (descriptor) {
    descriptor.emit('_valueWrite', null);
  } else {
...
Noble.prototype.onHandleRead = function(peripheralUuid, handle, data) {
  ...
  if (peripheral) {
    peripheral.emit('handleRead' + handle, data); // no internal event
  } else {
...
Noble.prototype.onHandleWrite = function(peripheralUuid, handle) {
  ...
  if (peripheral) {
    peripheral.emit('handleWrite' + handle); // no internal event
  } else {
...
Noble.prototype.onHandleNotify = function(peripheralUuid, handle, data) {
  ...
  if (peripheral) {
    peripheral.emit('handleNotify', handle, data); // no internal event
  } else {
