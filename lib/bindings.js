...
nobleBindings.connect = function(uuid) {
  this.sendCBMsg(9, {
    kCBMsgArgOptions: {
...
nobleBindings.disconnect = function(uuid) {
  this.sendCBMsg(10, {
    kCBMsgArgPeripheralHandle: this.peripherals[uuid].handle
  });
};
...
nobleBindings.updateRssi = function(uuid) {
  this.sendCBMsg(16, {
    kCBMsgArgPeripheralHandle: this.peripherals[uuid].handle
  });
};
...
nobleBindings.discoverServices = function(uuid, uuids) {
  var args = {
    kCBMsgArgPeripheralHandle: this.peripherals[uuid].handle,
    kCBMsgArgUUIDs: []
  };
...
nobleBindings.discoverIncludedServices = function(uuid, serviceUuid, serviceUuids) {
  var args = {
    kCBMsgArgPeripheralHandle: this.peripherals[uuid].handle,
...
nobleBindings.discoverCharacteristics = function(uuid, serviceUuid, characteristicUuids) {
  var args = {
    kCBMsgArgPeripheralHandle: this.peripherals[uuid].handle,
    kCBMsgArgServiceStartHandle: this.peripherals[uuid].services[serviceUuid].startHandle,
    kCBMsgArgServiceEndHandle: this.peripherals[uuid].services[serviceUuid].endHandle,
...
nobleBindings.read = function(uuid, serviceUuid, characteristicUuid) {
  this.sendCBMsg(29, {
    kCBMsgArgPeripheralHandle: this.peripherals[uuid].handle,
...
nobleBindings.on('kCBMsgId35', function(args) {
  ...
  for(var i in this.peripherals[handle].services) {
    if (this.peripherals[handle].services[i].characteristics &&
      ...
      this.emit('read', uuid, this.peripherals[handle].services[i].uuid,
...
nobleBindings.write = function(uuid, serviceUuid, characteristicUuid, data, notify) {
  this.sendCBMsg(30, {
    kCBMsgArgPeripheralHandle: this.peripherals[uuid].handle,
    kCBMsgArgCharacteristicHandle: this.peripherals[uuid].services[serviceUuid].characteristics[characteristicUuid].handle,
    kCBMsgArgCharacteristicValueHandle: this.peripherals[uuid].services[serviceUuid].characteristics[characteristicUuid].valueHandle,
...
nobleBindings.on('kCBMsgId36', function(args) {
  ...
  for(var i in this.peripherals[handle].services) {
    if (this.peripherals[handle].services[i].characteristics &&
        this.peripherals[handle].services[i].characteristics[characteristicHandle]) {
      this.emit('write', uuid, this.peripherals[handle].services[i].uuid,
...
nobleBindings.broadcast = function(uuid, serviceUuid, characteristicUuid, broadcast) {
  this.sendCBMsg(31, {
    kCBMsgArgPeripheralHandle: this.peripherals[uuid].handle,
    kCBMsgArgCharacteristicHandle: this.peripherals[uuid].services[serviceUuid].characteristics[characteristicUuid].handle,
    kCBMsgArgCharacteristicValueHandle: this.peripherals[uuid].services[serviceUuid].characteristics[characteristicUuid].valueHandle,
...
nobleBindings.on('kCBMsgId37', function(args) {
  ...
  for(var i in this.peripherals[handle].services) {
    if (this.peripherals[handle].services[i].characteristics &&
        this.peripherals[handle].services[i].characteristics[characteristicHandle]) {
      this.emit('broadcast', uuid, this.peripherals[handle].services[i].uuid,
...
nobleBindings.notify = function(uuid, serviceUuid, characteristicUuid, notify) {
  this.sendCBMsg(32, {
    kCBMsgArgPeripheralHandle: this.peripherals[uuid].handle,
    kCBMsgArgCharacteristicHandle: this.peripherals[uuid].services[serviceUuid].characteristics[characteristicUuid].handle,
    kCBMsgArgCharacteristicValueHandle: this.peripherals[uuid].services[serviceUuid].characteristics[characteristicUuid].valueHandle,
...
nobleBindings.on('kCBMsgId38', function(args) {
  ...
  for(var i in this.peripherals[handle].services) {
    if (this.peripherals[handle].services[i].characteristics &&
        this.peripherals[handle].services[i].characteristics[characteristicHandle]) {
      this.emit('notify', uuid, this.peripherals[handle].services[i].uuid,
        this.peripherals[handle].services[i].characteristics[characteristicHandle].uuid, state);
      break;
    }
  }
...
nobleBindings.discoverDescriptors = function(uuid, serviceUuid, characteristicUuid) {
  this.sendCBMsg(34, {
    kCBMsgArgPeripheralHandle: this.peripherals[uuid].handle,
    kCBMsgArgCharacteristicHandle: this.peripherals[uuid].services[serviceUuid].characteristics[characteristicUuid].handle,
    kCBMsgArgCharacteristicValueHandle: this.peripherals[uuid].services[serviceUuid].characteristics[characteristicUuid].valueHandle
...
nobleBindings.on('kCBMsgId39', function(args) {
  ...
  for(var i in this.peripherals[handle].services) {
    if (this.peripherals[handle].services[i].characteristics &&
      ...
      this.emit('descriptorsDiscover', uuid, this.peripherals[handle].services[i].uuid,
...
nobleBindings.readValue = function(uuid, serviceUuid, characteristicUuid, descriptorUuid) {
  this.sendCBMsg(40, {
    kCBMsgArgPeripheralHandle: this.peripherals[uuid].handle,
  ...
  for(var i in this.peripherals[handle].services) {
    for(var j in this.peripherals[handle].services[i].characteristics) {
      if (this.peripherals[handle].services[i].characteristics[j].descriptors &&
        ...
        this.emit('valueRead', uuid, this.peripherals[handle].services[i].uuid,
  ...
  for(var i in this.peripherals[handle].services) {
    for(var j in this.peripherals[handle].services[i].characteristics) {
      if (this.peripherals[handle].services[i].characteristics[j].descriptors &&
        ...
        this.emit('valueWrite', uuid, this.peripherals[handle].services[i].uuid,
          this.peripherals[handle].services[i].characteristics[j].uuid,
