...
nobleBindings.on('kCBMsgId35', function(args) {
  ...
  for(var i in this._peripherals[peripheralHandle].services) {
    if (this._peripherals[peripheralHandle].services[i].characteristics &&
      ...
      this.emit('read', peripheralUuid, this._peripherals[peripheralHandle].services[i].uuid,
        this._peripherals[peripheralHandle].services[i].characteristics[characteristicHandle].uuid, data, isNotification);
      break;
    }
  }
});
...
nobleBindings.writeValue = function(uuid, serviceUuid, characteristicUuid, descriptorUuid, data) {
  this.sendCBMsg(41, {
    kCBMsgArgPeripheralHandle: this._peripherals[uuid].handle,
    kCBMsgArgDescriptorHandle: this._peripherals[uuid].services[serviceUuid].characteristics[characteristicUuid].descriptors[descriptorUuid].handle,
    kCBMsgArgData: data
  });
};
